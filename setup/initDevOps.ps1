...
$environment = "shared"  # Adjust based on the environment
$spName = "sp-$projectName-$environment-devops"
$selfHostAgentName = "vm-$projectname-$environment-agent-1"
...
# Apply security policies - Put more policies here if you wish
Write-Host "Applying security policies to resource group $resourceGroupName"
az policy assignment create --name 'Require-Tagging' --policy '/providers/Microsoft.Authorization/policyDefinitions/1e30110a-5ceb-460c-a204-c1c3969c6d62' --scope "/subscriptions/$subscriptionId" --params '{"tagName": {"value": "environment"}, "tagValue": {"value": "shared"}}'

# Assign roles based on the ACL (assuming users already exist)
Write-Host "Reading acl.json file for user roles"
$acl = Get-Content -Raw -Path "acl.json" | ConvertFrom-Json

foreach ($user in $acl.users) {
    $email = $user.email
    $role = $user.role

    Write-Host "Assigning role $role to user $email"
    az role assignment create --assignee $email --role "$role" --scope "/subscriptions/$subscriptionId/resourceGroups/$resourceGroupName"
}
